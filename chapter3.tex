...
приложение~Г, рисунок~Г.1) ИИ-методист взаимодействует со~следующими
...
в~приложении~Г (рисунок~Г.3) и~описана в~разделе~3.2.
...
промежуточные артефакты (blueprint, список модулей, контент уроков)
...
\begin{enumerate}
  ...
  \item \textbf{CourseStructureAgent} --- генерирует структуру курса
    (Blueprint) в~соответствии с~выбранной методологией (ADDIE / SAM / BD)
  ...
  \item \textbf{FactCheckAgent} --- верифицирует фактические утверждения
    ...
    выявляет галлюцинации и~инициирует пересмотр при~низком faithfulness;
...
\subsection{Document Ingestion Pipeline (Haystack~2.x)}
...
\begin{enumerate}
  ...
  \item Уроки с~faithfulness ниже порога (по~умолчанию 0.8) помечаются
    ...
    на~перегенерацию;
  \item Human-in-the-loop уведомляет владельца курса о~изменённых разделах.
\end{enumerate}
